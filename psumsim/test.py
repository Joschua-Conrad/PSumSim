...
class test_Psum_Quantization(ut.TestCase):
	...
	def test_debugSimulation(self):
		...
		#Combine all testcases.
		cases = itertools.product(GROUPS, LEVELS, NUMMACS)
		...
		for groupsshapeshistaxesgroupnames, levels, nummacs in cases:
			...
			with self.subTest(
					groups=groups,
					shapesfromgroups=shapesfromgroups,
					**levels,
					nummacs=nummacs,
			):
				...
				checkedruns = (
						...
						(retstatistic, "retstatistic", AVERAGEOVER, True, False, "int"),
						(retstatisticdummy, "retstatisticdummy", AVERAGEOVER, False, False, "bool"),
						(retfullscalestoc, "retfullscalestoc", 1, True, False, "int"),
						(retfullscalestat, "retfullscalestat", 1, True, False, "int"),
				)

				checkedrunsfields = itertools.product(checkedruns, CHECKEDFIELDS)
				for therun, thefield in checkedrunsfields:
					...
					shapelookup = {
							...
							SHAPE_ENUM.HIST_CHUNK_SIZE : min((levels["activationlevels"] * levels["weightlevels"]), 35),
							...
							#residual elems is
							SHAPE_ENUM.MAC_CHUNKED_MERGED_ENUM : round(5 * 0.5) * (nummacchunks - 1) + round((nummacs - ((nummacchunks - 1) * 5)) * 0.5),
					...
					fieldsupportscomparison = (expectedshapes is None) and (not SKIP_GROUP_COMPARISON)
						...
						with self.subTest(
							...
							if fieldsupportscomparison:
								for comparegroupname in runcomparegroupnames:
									...
									reference = resultsstock[str(levels)]
									reference = reference[str(nummacs)]
									reference = reference[runname][comparegroupname]
				...
				if (effectivemergevalue is not None) and (effectivecliplimitfixed is not None) and (retstatistic["results"][-1].size == AVERAGEOVER) and ((effectivemergevalue == 1) or (effectivecliplimitfixed == 1)):
					...
					checkedruns = (
							(retstatistic, "retstatistic", AVERAGEOVER,),
					...
					for runobj, runname, effectiveaverageover in checkedruns:
						with self.subTest(runname=runname):
							...
							maxhistvalue = np.squeeze(maxhistvalue, axis=histax)
